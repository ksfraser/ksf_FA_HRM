...
use ksfraser\FrontAccounting\HRM\Service\EmploymentStatusService;
use Ksfraser\HTML\Elements\HtmlOption;

/**
 * Unit tests for EmploymentStatusService.
 *
...
class EmploymentStatusServiceTest extends TestCase
{
    ...
    {
        EmploymentStatusService::invalidateCache();
    ...
    public function testGetHtmlOptionsCacheExcludesSelectedId(): void
    {
        ...
        $cacheState = EmploymentStatusService::getOptionCacheState();
    ...
    public function testInvalidateCacheClearsAllLayers(): void
    {
        ...
        $this->service->getHtmlOptions();
        EmploymentStatusService::invalidateCache();
        $this->assertNull(EmploymentStatusService::getOptionCacheState());
    }

    // ─── Hook Response Methods ──────────────────────────────────────

    public function testHookGetEmploymentStatusesReturnsArrays(): void
    {
        $data = ['active_only' => true];
        $result = $this->service->hookGetEmploymentStatuses($data);
        $this->assertIsArray($result);
        $this->assertCount(2, $result);
        $this->assertArrayHasKey('status_id', $result[0]);
        $this->assertArrayHasKey('status_name', $result[0]);
    }

    public function testHookGetEmploymentStatusDdlReturnsStrings(): void
    {
        $data = ['active_only' => true, 'blank_label' => '-- Pick --'];
        $result = $this->service->hookGetEmploymentStatusDDL($data);
        $this->assertIsArray($result);
        $this->assertCount(3, $result);
        $this->assertStringContainsString('-- Pick --', $result[0]);
    }

    public function testHookGetEmploymentStatusHtmlOptionsReturnsOptionObjects(): void
    {
        $data = ['active_only' => true];
        $result = $this->service->hookGetEmploymentStatusHtmlOptions($data);
        $this->assertIsArray($result);
        $this->assertCount(2, $result);
    ...
    {
        $rendered = $this->service->getDdl(true, '-- Select Status --');
        $this->assertStringContainsString('value=""', $rendered[0]);
        $this->assertStringContainsString('-- Select Status --', $rendered[0]);
    }
