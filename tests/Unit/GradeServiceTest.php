...
class GradeServiceTest extends TestCase
{
    ...
    public function testGetHtmlOptionsCacheExcludesSelectedId(): void
    {
        ...
        $cacheState = GradeService::getOptionCacheState();
    ...
    public function testInvalidateCacheClearsAllLayers(): void
    {
        ...
        $this->service->getHtmlOptions();
        GradeService::invalidateCache();
        $this->assertNull(GradeService::getOptionCacheState());
    }
    ...
    public function testUpdateInvalidatesCache(): void
    {
        $this->service->getEntities();
        $this->service->getHtmlOptions();
        $this->service->update(1, ['grade_name' => 'Junior+']);
        $this->assertNull(GradeService::getOptionCacheState());
    }

    // ─── Hook Response Methods ──────────────────────────────────────

    public function testHookGetGradesReturnsArrays(): void
    {
        $data = ['active_only' => true];
        $result = $this->service->hookGetGrades($data);
        $this->assertIsArray($result);
        $this->assertCount(2, $result);
        $this->assertArrayHasKey('grade_id', $result[0]);
        $this->assertArrayHasKey('grade_name', $result[0]);
    }

    public function testHookGetGradeDdlReturnsStrings(): void
    {
        $data = ['active_only' => true, 'blank_label' => '-- Pick --'];
        $result = $this->service->hookGetGradeDDL($data);
        $this->assertIsArray($result);
        $this->assertCount(3, $result);
        $this->assertStringContainsString('-- Pick --', $result[0]);
    }

    public function testHookGetGradeHtmlOptionsReturnsOptionObjects(): void
    {
        $data = ['active_only' => true];
        $result = $this->service->hookGetGradeHtmlOptions($data);
        $this->assertIsArray($result);
        $this->assertCount(2, $result);
    ...
    {
        $rendered = $this->service->getDdl(true, '-- Select Grade --');
        $this->assertStringContainsString('value=""', $rendered[0]);
        $this->assertStringContainsString('-- Select Grade --', $rendered[0]);
    }
